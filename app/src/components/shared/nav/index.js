  ...
  return (
    <>
      ...
      <nav className={`app-nav ${menuIsOpen ? 'active' : ''}`}>
        <a href="/" className="d-block d-md-none mb-5">
        ...
        <ul>
          ...
          <li><NavLink onClick={closeMenu} className={({ isActive }) => (isActive ? 'active' : '')} disabled={true} to="/nft-feed"><img width="31" height="30" src="/assets/images/nav-nft.svg" />&nbsp;NFT Price Feed</NavLink></li>
          <li><NavLink onClick={closeMenu} className={({ isActive }) => (isActive ? 'active' : '')} disabled={true} to="/predictions"><img width="31" height="30" src="/assets/images/nav-prediction.svg" />&nbsp;Predictions</NavLink></li>
        </ul>
      </nav>
    </>
  );
